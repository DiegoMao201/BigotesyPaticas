...
class ManualStoryCreate(BaseModel):
    ...
    template_code: str | None = "behind_scenes_tienda"
    swipe_up_url: str | None = None
...
async def get_stories_config(
    ...
    rows = await db.execute(
        text("SELECT key, value, description FROM content.engine_config WHERE key LIKE 'stories%'")
    )
...
async def update_stories_config(
    ...
    allowed = {
        "stories_active",
        "stories_dry_run_mode",
        "stories_per_day",
        "stories_fallback_enabled",
    }
    if key not in allowed:
        raise HTTPException(400, f"Clave no permitida: {key}")
    value = str(body.get("value", ""))
    await db.execute(
        text("UPDATE content.engine_config SET value=:v, updated_at=NOW() WHERE key=:k"),
        {"v": value, "k": key},
    )
    await db.commit()
...
async def create_manual_story(
    ...
    """),
        {
            "id": story_id,
            "tc": body.template_code,
            "vu": body.video_url,
            "cap": body.caption,
            "suu": body.swipe_up_url,
...
async def delete_story(
    ...
    row = await db.execute(
        text("SELECT status FROM content.story_posts WHERE id=:id"), {"id": story_id}
    )
    r = row.fetchone()
    if not r:
        raise HTTPException(404, "Story no encontrada")
