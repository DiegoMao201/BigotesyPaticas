...
async def generate_post(payload: GeneratePostRequest, db: DBSession):
    ...
    cfg = (
        await db.execute(text("SELECT value FROM content.engine_config WHERE key = 'is_active'"))
    ).scalar_one_or_none()
    if cfg == "false":
        raise HTTPException(
            status_code=503,
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error generando post: {e}") from e
...
async def list_posts(
    ...
    total = (
        await db.execute(text(f"SELECT COUNT(*) FROM content.scheduled_posts {where}"), params)
    ).scalar_one()
...
async def regenerate_image(post_id: uuid.UUID, payload: RegenerateImageRequest, db: DBSession):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error regenerando imagen: {e}") from e
...
async def regenerate_with_model(
    post_id: uuid.UUID, payload: RegenerateWithModelRequest, db: DBSession
):
    ...
    gen = ContentGenerator()
    try:
        cdn_url, local_path, cost = await gen.regenerate_image_with_model(
            prompt, payload.image_model
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error regenerando imagen: {e}") from e
...
async def test_publish(payload: TestPublishRequest, db: DBSession):
    ...
    dry = (
        await db.execute(text("SELECT value FROM content.engine_config WHERE key = 'dry_run_mode'"))
    ).scalar_one_or_none()
...
async def list_templates(db: DBSession):
    rows = (
        (
            await db.execute(
                text(
                    "SELECT id, code, name, category, visual_style, cta_type, active FROM content.post_templates ORDER BY category, name"
                )
            )
        )
        .mappings()
...
async def trigger_week_plan(db: DBSession):
    ...
    proc = await asyncio.create_subprocess_exec(
        sys.executable,
        "/app/scripts/plan_content_week.py",
