...
def upgrade() -> None:
    ...
    op.create_table(
        "supplier_sku_map",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("supplier_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("purchasing.suppliers.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_supplier_sku_map_sku", "supplier_sku_map", ["sku_proveedor"], schema="purchasing")
