...
def upgrade() -> None:
    ...
    op.execute(text(
        "ALTER TABLE portal.portal_orders ADD COLUMN IF NOT EXISTS invoice_pdf_url TEXT NULL"
    ))
    op.execute(text(
    ...
    op.execute(text(
        "ALTER TABLE portal.portal_orders ADD COLUMN IF NOT EXISTS sales_order_id UUID NULL"
    ))

    # Expand portal_orders status CHECK (drop old, add new with 'invoiced')
    op.execute(text(
        "ALTER TABLE portal.portal_orders DROP CONSTRAINT IF EXISTS ck_portal_orders_status"
    ))
    op.execute(text(
    ...
    op.execute(text(
        "ALTER TABLE portal.appointments ADD COLUMN IF NOT EXISTS cancel_reason TEXT NULL"
    ))

    # ── portal.pets — thumbnail ───────────────────────────────────────────────
    op.execute(text(
        "ALTER TABLE portal.pets ADD COLUMN IF NOT EXISTS thumb_url TEXT NULL"
    ))

    # ── portal.notifications — nueva columna ──────────────────────────────────
    op.execute(text(
        "ALTER TABLE portal.notifications ADD COLUMN IF NOT EXISTS action_url TEXT NULL"
    ))
    ...
    op.execute(text(
        "ALTER TABLE crm.customers ADD COLUMN IF NOT EXISTS referral_code VARCHAR(20) NULL"
    ))
    op.execute(text(
    ...
    op.execute(text(
        "ALTER TABLE crm.customers ADD COLUMN IF NOT EXISTS referred_by_code VARCHAR(20) NULL"
    ))
    op.execute(text(
        "ALTER TABLE crm.customers ADD COLUMN IF NOT EXISTS referred_by_customer_id UUID NULL"
    ))
    ...
    op.execute(text(
        ...
        "ON portal.referrals(referral_code)"
    ))
...
def downgrade() -> None:
    ...
    op.execute(text("ALTER TABLE portal.portal_orders DROP CONSTRAINT IF EXISTS ck_portal_order_status"))
    op.execute(text(
