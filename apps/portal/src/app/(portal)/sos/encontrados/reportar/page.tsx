...
import { rescues, type RescueEventInput } from '@/lib/api';
...
export default function ReportRescuePage() {
  ...
  const [form, setForm] = useState<Partial<RescueEventInput>>({
  ...
  const set = <K extends keyof RescueEventInput>(k: K, v: RescueEventInput[K]) =>
    setForm((f) => ({ ...f, [k]: v }));

  const { mutateAsync: createEvent } = useMutation({
    mutationFn: (data: RescueEventInput) => rescues.report(data),
  });
  ...
  function onFilesSelected(files: FileList | null) {
    ...
    const next: Draft[] = Array.from(files)
      ...
      .map((file) => ({ file, preview: URL.createObjectURL(file), description: '' }));
    setDrafts((prev) => [...prev, ...next]);
  }

  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    if (!form.title?.trim()) return toast.error('Escribe un título, ej: "3 gatitos encontrados en el parque"');
    ...
    setSubmitting(true);
    try {
      const event = await createEvent({ ...(form as RescueEventInput), lat: loc.lat, lng: loc.lng });
      setCreatedEventId(event.id);
      toast.success('Reporte creado — ahora sube las fotos');
    } catch (err) {
  ...
  async function handlePublish() {
    ...
    setUploading(true);
    try {
      await rescues.uploadAnimals(
        createdEventId,
        drafts.map((d) => d.file),
        drafts.map((d) => d.description)
      );
      toast.success('🏠 ¡Publicado! Gracias por ayudar a que encuentren su hogar');
      router.push(`/sos/encontrados/${createdEventId}`);
    } catch (err) {
  ...
  return (
    <div className="p-4 pt-6 flex flex-col gap-5 pb-8">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-display text-xl font-bold text-foreground">Reportar animalito encontrado</h1>
      </div>

      <div className="rounded-2xl p-4 bg-[#E6F5F1] border border-[#bfe6da]">
        <p className="text-xs text-emerald-800 font-medium leading-relaxed">
          🏠 Publica las fotos para que la comunidad pueda reconocer si es su mascota y contactarte.
        </p>
      </div>
        ...
        <form onSubmit={handleCreate} className="flex flex-col gap-5">
          <div>
            ...
            <input
              ...
              placeholder='Ej: "3 gatitos encontrados cerca al parque"'
              value={form.title ?? ''}
          ...
          <div>
            ...
            <textarea
              className="input-field"
              rows={3}
              placeholder="Cómo los encontraste, en qué condiciones están, dónde están ahora… (opcional)"
          ...
          <div>
            ...
            <input
              className="input-field"
              placeholder="Ej: Barrio Cuba, cerca al parque"
              value={form.address ?? ''}
              ...
              {drafts.map((d, i) => (
                <div key={i} className="relative">
                  ...
                  <input
                    ...
                    className="mt-1 w-full text-[10px] rounded-lg border border-border px-1.5 py-1"
                  />
                </div>
              ))}
              <button
