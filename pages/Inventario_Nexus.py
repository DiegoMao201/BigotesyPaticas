...
def main():
    ...
    with tabs[0]:
        ...
        # solo columnas que existen (blindaje final)
...
# ==========================================
# 6. FUNCIONES DE SOPORTE UI
# ==========================================

def descargar_excel_conteo(df: pd.DataFrame) -> bytes:
    """Genera Excel para conteo físico."""
    output = io.BytesIO()
    try:
...
def crear_orden_compra(proveedor: str, items_df: pd.DataFrame) -> str:
    """Registra orden en Historial_Ordenes y retorna ID."""
    try:
        data = st.session_state.get("data_store", {})
...
def analizar_ventas(df_ven: pd.DataFrame, df_inv: pd.DataFrame) -> dict:
    """
    Retorna dict: {id_norm: {v90, v30}} con unidades vendidas en 90 y 30 días.
    """
    stats = {}
    try:
...
# ✅ SIEMPRE llamar main() (Streamlit corre el archivo completo)
main()