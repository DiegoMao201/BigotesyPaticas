...
def calcular_master_df() -> pd.DataFrame:
    """
    Motor analítico completo. NUNCA lanza KeyError/AttributeError.
    Retorna master_df con TODAS las columnas garantizadas.
    """
    ...
    # Costo efectivo (proveedor si existe, sino inventario)
    ...
    # ── 6. MODO DEMANDA ───────────────────────────────────────────────────
    # ARRANQUE = 1 venta aislada (no proyectar, solo mínimo 1 unidad)
    # ROTACION = data suficiente para calcular velocidad
    ...
    # ── 7. VELOCIDAD DIARIA (suavizada) ───────────────────────────────────
    vel_30    = master["v30"] / 30.0
    vel_90    = master["v90"] / 90.0
    vel_blend = (0.65 * vel_90) + (0.35 * vel_30)
    conf      = np.clip(master["v90"] / 6.0, 0.0, 1.0)   # confianza estadística
    ...
    master["Velocidad_Diaria"] = pd.to_numeric(
        master["Velocidad_Diaria"], errors="coerce"
    ).fillna(0.0)
    ...
    # ARRANQUE usa Factor_Pack=1 para no inflar (1 caja de 5 por 1 venta = incorrecto)
    ...
    faltante_rot = np.maximum(
        0.0, np.maximum(stock_obj, master["Min_Unidades"]) - master["Stock"]
    )
    ...
    master["Sugerencia_Cajas"] = np.ceil(
        master["Faltante"] / master["Factor_Pack_Efectivo"]
    )
    master["Unidades_Pedir"] = (
        master["Sugerencia_Cajas"] * master["Factor_Pack_Efectivo"]
    )
    ...
    # ── 12. GARANTÍA FINAL (columnas que usa main()) ───────────────────────
    cols_garantizadas = {
        "ID_Producto": "", "Nombre": "", "Categoria": "Sin Categoría",
...
def main():
    with st.sidebar:
        ...
        st.info(
            f"Última sinc: "
            f"{st.session_state.get('last_sync', datetime.min).strftime('%H:%M:%S')}"
        )
        ...
        if "data_store" in st.session_state:
            ...
            if col_cat_sb:
                ...
                cat_filter = st.multiselect("Filtrar por Categoría", sorted(cats), default=[])
            abc_filter = st.multiselect("Clasificación ABC", ["A", "B", "C"], default=[])

    if "data_store" not in st.session_state:
        st.error("No hay datos cargados. Usa 🔄 Forzar Sincronización.")
        return
    ...
    margen_promedio = (
        float(master_df.loc[master_df["Stock"] > 0, "Margen_%"].mean() or 0.0) * 100.0
    )

    df_gastos   = st.session_state["data_store"].get("df_Gastos", pd.DataFrame())
    gastos_mes  = 0.0
    if not df_gastos.empty and "Fecha" in df_gastos.columns and "Monto" in df_gastos.columns:
        ...
        gastos_mes = float(
            df_gastos[df_gastos["Fecha"].dt.month == datetime.now().month]["Monto"].sum()
        )
    ...
    with tabs[0]:
        ...
        # ✅ Tabla: solo columnas garantizadas
        cols_vista = [
    ...
    with tabs[1]:
        ...
        st.info(
            ...
            "Productos con 1 venta aislada → modo **ARRANQUE** (solo 1 unidad mínima, sin inflar cajas)."
        )
            ...
            for prov in seleccion["Nombre_Proveedor"].unique():
                ...
                with st.expander(
                    ...
                    if c_btn.button(f"Emitir Orden ({prov})", key=f"btn_{prov}"):
                        ...
                        time.sleep(1.5)
                        st.rerun()
                    msg = f"Hola {prov}, pedido Bigotes & Patitas:\n"
                    for _, r in items_prov.iterrows():
                        msg += f"- {int(r['Sugerencia_Cajas'])} cajas de {r['Nombre']}\n"
                    link = f"https://wa.me/?text={quote(msg)}"
                    c_wa.markdown(f"*(Opcional)* [📲 Enviar por WhatsApp]({link})")
    ...
    with tabs[3]:
        ...
        with col_g1:
            ...
            top_m = master_df.sort_values("Margen_$", ascending=False).head(5)
            st.dataframe(top_m[["Nombre", "Costo", "Precio", "Margen_$"]], hide_index=True)
        with col_g2:
            st.markdown("**Resumen de Gastos**")
            if not df_gastos.empty and "Categoria" in df_gastos.columns:
                st.dataframe(
                    df_gastos.groupby("Categoria")["Monto"].sum().reset_index(),
                    hide_index=True,
                )
            else:
