...
def cargar_datos():
    ...
    df_hist = ws_to_df(
        get_ws_safe(sh, "Historial_Recepciones"),
        {
            "Fecha": "",
            "Folio": "",
            "Proveedor": "",
    ...
    df_map = ws_to_df(
        get_ws_safe(sh, "Maestro_Proveedores"),
        {
            "Producto_UID": "",
            "SKU_Interno": "",
            "Nombre_Proveedor": "",
    ...
    if not df_map.empty:
        ...
        if "Factor_Pack" in df_map.columns:
            df_map["Factor_Pack"] = pd.to_numeric(df_map["Factor_Pack"], errors="coerce").fillna(
                1.0
            )
...
def expandir_ventas(df_ven: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, sale in df_ven.iterrows():
        ...
        for item in detalles:
            ...
            precio_unit = coalesce_number(
                item.get("Precio_Unitario"),
                item.get("Precio"),
                item.get("precio_unitario"),
                item.get("precio"),
            )
            descuento_unit = coalesce_number(
                item.get("Descuento_Unitario"),
                item.get("Descuento"),
                item.get("descuento_unitario"),
                item.get("descuento"),
            )
            costo_unit = coalesce_number(
                item.get("Costo_Unitario"),
                item.get("Costo"),
                item.get("costo_unitario"),
                item.get("costo"),
            )
            subtotal_linea = coalesce_number(
                item.get("Subtotal_Linea"),
                item.get("Subtotal"),
                item.get("subtotal_linea"),
                item.get("subtotal"),
            )
            ...
            if total_lineas == 1:
                ...
                if fuente_detalle != "Exacto" or (precio_unit > 0 and costo_unit > 0):
                    fuente_detalle = (
                        ...
                        if any(
                            k in item
                            for k in ["Precio_Unitario", "Precio", "Costo_Unitario", "Costo"]
                        )
                        else "Inferido de venta de una sola línea"
                    )
            ...
            margen_linea = (
                ingreso_linea - costo_total_linea
                if pd.notna(ingreso_linea) and pd.notna(costo_total_linea)
                else np.nan
            )
            ...
            rows.append(
                {
                    ...
                    "ID_Producto_Norm": normalizar_id_producto(
                        item.get("ID_Producto_Norm", item.get("ID", item.get("ID_Producto", "")))
                    ),
                    "Nombre_Producto": str(
                        item.get("Nombre", item.get("Nombre_Producto", ""))
                    ).strip(),
                    "Cantidad": qty,
...
def filtrar_producto(df: pd.DataFrame, producto_uid: str, sku_norm: str) -> pd.DataFrame:
    ...
    if sku_norm:
        if "ID_Producto_Norm" in df.columns:
            mask = mask | (
                df["ID_Producto_Norm"].astype(str).apply(normalizar_id_producto) == sku_norm
            )
        if "SKU_Interno_Norm" in df.columns:
            mask = mask | (
                df["SKU_Interno_Norm"].astype(str).apply(normalizar_id_producto) == sku_norm
            )
    return df[mask].copy()
...
def build_alerts(producto, compras_df, ventas_df, costo_ref_actual):
    ...
    ultimo_costo_compra = (
        money_float(compras_df["Costo_Unitario"].iloc[-1]) if not compras_df.empty else 0.0
    )
    ultimo_precio_venta = (
        money_float(ventas_df["Precio_Unitario"].dropna().iloc[-1])
        if not ventas_df["Precio_Unitario"].dropna().empty
        else 0.0
    )

    if (
        ultimo_costo_compra > 0
        and ultimo_precio_venta > 0
        and ultimo_precio_venta <= ultimo_costo_compra
    ):
        alerts.append(
            (
                "bad",
                "Última venta por debajo del último costo",
                f"Último precio facturado {money_fmt(ultimo_precio_venta)} vs último costo recibido {money_fmt(ultimo_costo_compra)}.",
            )
        )

    margen_actual = (
        ((precio_actual - costo_actual) / precio_actual)
        if precio_actual > 0 and costo_actual > 0
        else np.nan
    )
    if pd.notna(margen_actual) and margen_actual < 0.20:
        alerts.append(
            (
                "warn",
                "Margen actual estrecho",
                f"El margen actual del producto está en {pct_fmt(margen_actual)} sobre el precio vigente.",
            )
        )
    ...
    if costo_ref_actual > 0 and costo_actual > 0:
        ...
        if gap > 0.35:
            alerts.append(
                (
                    "warn",
                    "Costo inventario y proveedor muy separados",
                    f"Costo inventario {money_fmt(costo_actual)} vs referencia proveedor {money_fmt(costo_ref_actual)}.",
                )
            )
    ...
    if stock_actual > 0 and not ventas_df.empty:
        ...
        if pd.notna(ultima_venta) and ultima_venta < pd.Timestamp.now() - pd.Timedelta(days=45):
            alerts.append(
                (
                    "warn",
                    "Producto con stock y baja rotación reciente",
                    f"Tiene stock disponible, pero no registra ventas desde {ultima_venta.strftime('%Y-%m-%d')}.",
                )
            )
    ...
    if not alerts:
        alerts.append(
            (
                "ok",
                "Lectura sana del producto",
                "La trazabilidad principal está disponible y no se detectan alertas críticas inmediatas en costo, precio o rotación.",
            )
        )
...
def build_storyline(
    ...
    if compras_unidades > 0:
        story.append(
            f"{nombre} recibió {qty_fmt(compras_unidades)} unidades en el rango analizado, con una inversión acumulada de {money_fmt(compras_total)}."
        )
    else:
    ...
    if ventas_unidades > 0:
        story.append(
            f"Se facturaron {qty_fmt(ventas_unidades)} unidades por {money_fmt(ventas_total)} y el margen bruto realizado del periodo fue {money_fmt(margen_real)}."
        )
    else:
    ...
    if costo_ref_actual > 0:
        story.append(
            f"El costo de referencia del proveedor está en {money_fmt(costo_ref_actual)}, mientras que en inventario el costo vigente está en {money_fmt(costo_actual)}."
        )

    if costo_actual > 0 and precio_actual > 0:
        margen_actual = (precio_actual - costo_actual) / precio_actual if precio_actual > 0 else 0.0
        story.append(
            f"Con el precio actual de {money_fmt(precio_actual)}, el margen teórico vigente del producto es {pct_fmt(margen_actual)}."
        )
...
def main():
    ...
    sku_norm = normalizar_id_producto(
        producto.get("ID_Producto_Norm", producto.get("ID_Producto", ""))
    )
    ...
    mapa_producto = (
        filtrar_producto(df_map, producto_uid, sku_norm).sort_values("Ultima_Actualizacion")
        if not df_map.empty
        else pd.DataFrame()
    )
    ...
    default_start = (
        min(min_date_candidates).date()
        if min_date_candidates
        else (datetime.now() - timedelta(days=120)).date()
    )
    ...
    if not compras_producto.empty:
        compras_producto = compras_producto[
            (compras_producto["Fecha"] >= fecha_ini) & (compras_producto["Fecha"] <= fecha_fin)
        ].copy()
    if not ventas_producto.empty:
        ventas_producto = ventas_producto[
            (ventas_producto["Fecha"] >= fecha_ini) & (ventas_producto["Fecha"] <= fecha_fin)
        ].copy()

    costo_ref_actual = (
        money_float(mapa_producto["Costo_Proveedor"].iloc[-1]) if not mapa_producto.empty else 0.0
    )
    proveedor_actual = (
        str(mapa_producto["Nombre_Proveedor"].iloc[-1]).strip()
        if not mapa_producto.empty
        else "Sin asignar"
    )

    compras_unidades = (
        float(compras_producto["Unidades"].sum()) if not compras_producto.empty else 0.0
    )
    ventas_unidades = float(ventas_producto["Cantidad"].sum()) if not ventas_producto.empty else 0.0
    compras_total = (
        float(compras_producto["Costo_Total"].sum()) if not compras_producto.empty else 0.0
    )
    ventas_total = (
        float(ventas_producto["Ingreso_Linea"].fillna(0).sum())
        if not ventas_producto.empty
        else 0.0
    )
    costo_vendido = (
        float(ventas_producto["Costo_Total_Linea"].fillna(0).sum())
        if not ventas_producto.empty
        else 0.0
    )
    ...
    cobertura_precio = (
        float(
            (
                ventas_producto["Detalle_Fuente"].eq("Exacto")
                | ventas_producto["Detalle_Fuente"].eq("Inferido de venta de una sola línea")
            ).mean()
        )
        if not ventas_producto.empty
        else 0.0
    )
    utilidad_potencial_actual = (
        money_float(producto.get("Precio", 0)) - money_float(producto.get("Costo", 0))
    ) * money_float(producto.get("Stock", 0))
    rotacion_neta = ventas_unidades - compras_unidades
    avg_ticket_producto = (
        ventas_total / len(ventas_producto["ID_Venta"].dropna().unique())
        if not ventas_producto.empty and len(ventas_producto["ID_Venta"].dropna().unique()) > 0
        else 0.0
    )
    ...
    chips = [
        ...
        badge(
            f"Categoría {str(producto.get('Categoria', 'Sin Categoría')).strip() or 'Sin Categoría'}"
        ),
    ...
    render_metric_grid(
        [
            {
                "label": "Entradas",
                "value": qty_fmt(compras_unidades),
                "foot": f"{len(compras_producto)} movimientos de compra",
            },
            {
                "label": "Salidas",
                "value": qty_fmt(ventas_unidades),
                "foot": f"{len(ventas_producto)} líneas de venta",
            },
            {
                "label": "Costo Prom. Compra",
                "value": money_fmt(costo_prom_compra),
                "foot": f"Comprado por {money_fmt(compras_total)}",
            },
            {
                "label": "Precio Prom. Venta",
                "value": money_fmt(precio_prom_venta),
                "foot": f"Facturado por {money_fmt(ventas_total)}",
            },
            {
                "label": "Margen Realizado",
                "value": money_fmt(margen_real),
                "foot": pct_fmt(margen_pct),
            },
            {
                "label": "Cobertura Detalle Venta",
                "value": pct_fmt(cobertura_precio),
                "foot": "Qué tanto del historial tiene precio/costo unitario utilizable",
            },
        ]
    )
    ...
    render_metric_grid(
        [
            {
                "label": "Stock Actual",
                "value": qty_fmt(producto.get("Stock", 0)),
                "foot": f"Costo vigente {money_fmt(producto.get('Costo', 0))}",
            },
            {
                "label": "Precio Vigente",
                "value": money_fmt(producto.get("Precio", 0)),
                "foot": f"Costo proveedor {money_fmt(costo_ref_actual)}",
            },
            {
                "label": "Utilidad Potencial Stock",
                "value": money_fmt(utilidad_potencial_actual),
                "foot": "Con precio y costo vigentes",
            },
            {
                "label": "Rotación Neta",
                "value": qty_fmt(rotacion_neta),
                "foot": "Ventas menos compras del rango",
            },
            {
                "label": "Compradores Únicos",
                "value": qty_fmt(
                    ventas_producto["Nombre_Cliente"]
                    .fillna("")
                    .replace("", np.nan)
                    .dropna()
                    .nunique()
                ),
                "foot": "Clientes distintos del periodo",
            },
            {
                "label": "Ticket Prom. Producto",
                "value": money_fmt(avg_ticket_producto),
                "foot": "Promedio por venta donde apareció",
            },
        ]
    )
    ...
    with tabs[0]:
        ...
        with col_a:
            ...
            if compras_producto.empty and ventas_producto.empty:
                ...
                fig = fig_base(
                    fig, "Ritmo de movimiento, costo recibido y precio facturado", height=470
                )
        ...
        with col_b:
            ...
            penultima_venta = (
                ventas_producto.dropna(subset=["Precio_Unitario"]).iloc[-2]
                if len(ventas_producto.dropna(subset=["Precio_Unitario"])) >= 2
                else None
            )
            ...
            if ultima_compra is not None:
                ...
                if penultima_compra is not None:
                    delta_costo_val = money_float(
                        ultima_compra.get("Costo_Unitario", 0)
                    ) - money_float(penultima_compra.get("Costo_Unitario", 0))
                    delta_costo = f" | Delta {money_fmt(delta_costo_val)}"
                resumen_rows.append(
                    f"Última compra: {ultima_compra['Fecha'].strftime('%Y-%m-%d')} por {money_fmt(ultima_compra.get('Costo_Unitario', 0))}{delta_costo}"
                )
            if ultima_venta is not None and pd.notna(ultima_venta.get("Precio_Unitario", np.nan)):
                delta_precio = ""
                if penultima_venta is not None:
                    delta_precio_val = money_float(
                        ultima_venta.get("Precio_Unitario", 0)
                    ) - money_float(penultima_venta.get("Precio_Unitario", 0))
                    delta_precio = f" | Delta {money_fmt(delta_precio_val)}"
                resumen_rows.append(
                    f"Última venta: {ultima_venta['Fecha'].strftime('%Y-%m-%d')} por {money_fmt(ultima_venta.get('Precio_Unitario', 0))}{delta_precio}"
                )
            resumen_rows.append(
                f"Costo actual en inventario: {money_fmt(producto.get('Costo', 0))}"
            )
            resumen_rows.append(
                f"Precio actual en inventario: {money_fmt(producto.get('Precio', 0))}"
            )
            ...
            compras_cliente = (
                ventas_producto.groupby("Nombre_Cliente", dropna=False)
                .agg(
        ...
        with col_c1:
            ...
            if not compras_producto.empty:
                compra_daily = (
                    compras_producto.groupby(compras_producto["Fecha"].dt.date)
                    .agg(Costo_Compra=("Costo_Unitario", "mean"))
                    .reset_index()
                )
                fig_mix.add_trace(
                    go.Scatter(
                        x=compra_daily["Fecha"],
                        y=compra_daily["Costo_Compra"],
                        mode="lines+markers",
                        name="Costo compra",
                        line=dict(color=COLOR_PRIMARIO, width=3),
                    )
            ...
            if not ventas_producto.dropna(subset=["Precio_Unitario"]).empty:
                venta_daily = (
                    ventas_producto.dropna(subset=["Precio_Unitario"])
                    .groupby(ventas_producto.dropna(subset=["Precio_Unitario"])["Fecha"].dt.date)
                    .agg(Precio_Venta=("Precio_Unitario", "mean"))
                    .reset_index()
                )
                fig_mix.add_trace(
                    go.Scatter(
                        x=venta_daily["Fecha"],
                        y=venta_daily["Precio_Venta"],
                        mode="lines+markers",
                        name="Precio venta",
                        line=dict(color=COLOR_ACENTO, width=3, dash="dot"),
                    )
            ...
            fig_cash.add_trace(
                go.Bar(
                    ...
                    marker_color=[
                        COLOR_PRIMARIO,
                        COLOR_ACENTO,
                        COLOR_OK if margen_real >= 0 else COLOR_PELIGRO,
                    ],
                    text=[
                        money_fmt(compras_total),
                        money_fmt(ventas_total),
                        money_fmt(margen_real),
                    ],
                    textposition="outside",
                )
            )
            fig_cash = fig_base(
                fig_cash, "Dinero invertido, facturado y margen generado", height=360
            )
    ...
    with tabs[1]:
        ...
        incluir_sin_detalle = kardex_cols[1].checkbox(
            "Incluir ventas históricas sin detalle unitario", value=True
        )
        ...
        if not incluir_sin_detalle:
            ventas_k = ventas_k[
                ventas_k["Detalle_Fuente"].ne("Histórico sin detalle unitario")
            ].copy()
        ventas_k["Tipo"] = "Venta"
        ...
        if movimiento_tipo in ["Todos", "Compras"] and not compras_k.empty:
            frames.append(
                ...
                ].rename(columns={"Nombre_Producto": "Producto"})
            )
    ...
    with tabs[2]:
        ...
        with col_p:
            ...
            if compras_producto.empty:
                ...
                prov_df = (
                    compras_producto.groupby("Proveedor", dropna=False)
                    .agg(
                    ...
                    .reset_index()
                    .sort_values(["Entradas", "Inversion"], ascending=False)
                )
                st.dataframe(
                    prov_df,
                    use_container_width=True,
        ...
        with col_q:
            ...
            if ventas_producto.empty:
                ...
                cli_df = (
                    ...
                    .reset_index()
                    .sort_values(["Facturacion", "Unidades"], ascending=False)
                )
                st.dataframe(
                    cli_df.head(15),
                    use_container_width=True,
    ...
    with tabs[3]:
        ...
        for level, title, body in build_alerts(
            producto, compras_producto, ventas_producto, costo_ref_actual
        ):
            st.markdown(
                f"""
        ...
        diag = {
            ...
            "Última compra": compras_producto["Fecha"].max().strftime("%Y-%m-%d %H:%M")
            if not compras_producto.empty and pd.notna(compras_producto["Fecha"].max())
            else "Sin registro",
            "Última venta": ventas_producto["Fecha"].max().strftime("%Y-%m-%d %H:%M")
            if not ventas_producto.empty and pd.notna(ventas_producto["Fecha"].max())
            else "Sin registro",
    ...
    with tabs[4]:
        ...
        frecuencia = st.selectbox(
            "Frecuencia de análisis", ["Diaria", "Semanal", "Mensual"], index=2
        )
        ...
        if not compras_ev.empty:
            compras_ev["Periodo"] = (
                compras_ev["Fecha"]
                .dt.to_period("M" if freq == "M" else ("W" if freq == "W" else "D"))
                .dt.to_timestamp()
            )
            compras_agg = (
                compras_ev.groupby("Periodo")
                .agg(
            ...
            compras_agg = pd.DataFrame(
                columns=["Periodo", "Unidades_Compradas", "Costo_Comprado", "Costo_Unit_Prom"]
            )
        ...
        evo = (
            pd.merge(compras_agg, ventas_agg, on="Periodo", how="outer")
            .sort_values("Periodo")
            .fillna(0)
        )
        if evo.empty:
            st.info(
                "No hay suficiente historial para construir evolución del producto en este rango."
            )
        else:
            ...
            with col_e2:
                ...
                if "Costo_Unit_Prom" in evo.columns:
                    fig_evo2.add_trace(
                        go.Scatter(
                            x=evo["Periodo"],
                            y=evo["Costo_Unit_Prom"],
                            mode="lines+markers",
                            name="Costo unitario prom.",
                            line=dict(color=COLOR_PRIMARIO, width=3),
                        )
                    )
                if "Precio_Unit_Prom" in evo.columns:
                    fig_evo2.add_trace(
                        go.Scatter(
                            x=evo["Periodo"],
                            y=evo["Precio_Unit_Prom"],
                            mode="lines+markers",
                            name="Precio unitario prom.",
                            line=dict(color=COLOR_ACENTO, width=3, dash="dot"),
                        )
                    )
